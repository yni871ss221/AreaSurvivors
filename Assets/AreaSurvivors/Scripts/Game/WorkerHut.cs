...
namespace AreaSurvivors
{
    ...
    public sealed class WorkerHut : MonoBehaviour, IBuildableConstruction
    {
        ...
        void Start()
        {
            if (config != null)
            {
                buildSeconds = config.workerHutBuildSeconds;
                maxHp = config.workerHutMaxHp;
            }
        ...
        {
            var candidates = new List<HarvestableResource>();
            int bestDistance = int.MaxValue;
            foreach (var resource in FindObjectsOfType<HarvestableResource>())
            {
                if (resource == null || resource.resourceType != type || resource.remainingAmount <= 0 || resource.grid != grid) continue;
                if (!IsConnectedByPlayerTerritory(resource.originCell, resource.footprint)) continue;

                int distance = CellDistanceToFootprint(resource.originCell, resource.footprint);
        ...
        int CellDistanceToFootprint(Vector3Int targetOrigin, Vector2Int targetFootprint)
        {
            int best = int.MaxValue;
            foreach (var from in FootprintCells(OriginCell, Footprint))
            {
                foreach (var to in FootprintCells(targetOrigin, targetFootprint))
                {
                    best = Mathf.Min(best, Mathf.Abs(from.x - to.x) + Mathf.Abs(from.y - to.y));
                }
            }
