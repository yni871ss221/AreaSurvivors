...
namespace AreaSurvivors
{
    ...
    public sealed class TileGrid : MonoBehaviour
    {
        ...
        public bool showTerritoryBoundary = true;
        public Color territoryBoundaryColor = new Color(0.52f, 0.95f, 0.82f, 0.9f);
        public float territoryBoundaryThickness = 0.035f;
        public int territoryBoundarySortingOrder = 935;
        ...
        GameObject territoryBoundaryOverlay;
        Mesh territoryBoundaryMesh;
        Material territoryBoundaryMaterial;
        bool territoryBoundaryDirty;
        ...
        void Awake()
        {
            ...
            if (showTerritoryBoundary) CreateTerritoryBoundaryOverlay();
        }
        ...
        public void Build()
        {
            ...
            CreateTerritoryBoundaryOverlay();
            MarkTerritoryBoundaryDirty();
            UpdateTerritoryBoundaryOverlay();
        }
        ...
        Mesh BuildGridLineMesh()
        {
            ...
            for (int x = 0; x <= width; x++)
            {
                ...
                AddLineQuad(vertices, triangles, gridLineOverlay.transform, start, start + upStep * height, thickness);
            }

            for (int y = 0; y <= height; y++)
            {
                Vector3 start = bottomLeft + upStep * y;
                AddLineQuad(vertices, triangles, gridLineOverlay.transform, start, start + rightStep * width, thickness);
            }
        ...
        void AddLineQuad(List<Vector3> vertices, List<int> triangles, Transform overlayRoot, Vector3 worldStart, Vector3 worldEnd, float thickness)
        {
            Vector3 start = overlayRoot.InverseTransformPoint(worldStart);
            Vector3 end = overlayRoot.InverseTransformPoint(worldEnd);
        ...
        void CreateTerritoryBoundaryOverlay()
        {
            DestroyTerritoryBoundaryOverlay();
            if (!showTerritoryBoundary || groundTilemap == null || width <= 0 || height <= 0) return;

            territoryBoundaryOverlay = new GameObject("Territory Boundary Overlay");
            territoryBoundaryOverlay.transform.SetParent(transform, false);

            var meshFilter = territoryBoundaryOverlay.AddComponent<MeshFilter>();
            var meshRenderer = territoryBoundaryOverlay.AddComponent<MeshRenderer>();
            var shader = Shader.Find("Sprites/Default");
            if (shader == null) shader = Shader.Find("Unlit/Transparent");
            territoryBoundaryMaterial = new Material(shader);
            territoryBoundaryMaterial.color = territoryBoundaryColor;
            meshRenderer.sharedMaterial = territoryBoundaryMaterial;
            meshRenderer.sortingOrder = territoryBoundarySortingOrder;

            territoryBoundaryMesh = new Mesh();
            territoryBoundaryMesh.name = "Territory Boundary Overlay Mesh";
            territoryBoundaryMesh.MarkDynamic();
            meshFilter.sharedMesh = territoryBoundaryMesh;
            territoryBoundaryDirty = true;
        }

        void MarkTerritoryBoundaryDirty()
        {
            if (!showTerritoryBoundary) return;
            territoryBoundaryDirty = true;
        }

        void UpdateTerritoryBoundaryOverlay()
        {
            if (!showTerritoryBoundary)
            {
                DestroyTerritoryBoundaryOverlay();
                return;
            }

            if (territoryBoundaryOverlay == null)
            {
                CreateTerritoryBoundaryOverlay();
            }

            if (territoryBoundaryOverlay == null || !territoryBoundaryDirty || owners == null || groundTilemap == null) return;

            var meshRenderer = territoryBoundaryOverlay.GetComponent<MeshRenderer>();
            if (meshRenderer != null) meshRenderer.sortingOrder = territoryBoundarySortingOrder;
            if (territoryBoundaryMaterial != null) territoryBoundaryMaterial.color = territoryBoundaryColor;

            RebuildTerritoryBoundaryMesh();
            territoryBoundaryDirty = false;
        }

        void RebuildTerritoryBoundaryMesh()
        {
            if (territoryBoundaryMesh == null || territoryBoundaryOverlay == null) return;

            var vertices = new List<Vector3>();
            var triangles = new List<int>();
            float thickness = Mathf.Max(0.001f, territoryBoundaryThickness);
            Vector3 firstCenter = groundTilemap.GetCellCenterWorld(GridToCell(0, 0));
            Vector3 rightStep = groundTilemap.GetCellCenterWorld(GridToCell(1, 0)) - firstCenter;
            Vector3 upStep = groundTilemap.GetCellCenterWorld(GridToCell(0, 1)) - firstCenter;
            if (width == 1) rightStep = new Vector3(cellSize, 0f, 0f);
            ...
            for (int y = 0; y < height; y++)
            {
                for (int x = 0; x < width; x++)
                {
                    if (!IsPlayerOwned(x, y)) continue;

                    Vector3 cellBottomLeft = bottomLeft + rightStep * x + upStep * y;
                    Vector3 cellBottomRight = cellBottomLeft + rightStep;
                    Vector3 cellTopLeft = cellBottomLeft + upStep;
                    Vector3 cellTopRight = cellTopLeft + rightStep;

                    if (!IsPlayerOwned(x - 1, y))
                    {
                        AddLineQuad(vertices, triangles, overlayRoot, cellBottomLeft, cellTopLeft, thickness);
                    }

                    if (!IsPlayerOwned(x + 1, y))
                    {
                        AddLineQuad(vertices, triangles, overlayRoot, cellBottomRight, cellTopRight, thickness);
                    }

                    if (!IsPlayerOwned(x, y - 1))
                    {
                        AddLineQuad(vertices, triangles, overlayRoot, cellBottomLeft, cellBottomRight, thickness);
                    }

                    if (!IsPlayerOwned(x, y + 1))
                    {
                        AddLineQuad(vertices, triangles, overlayRoot, cellTopLeft, cellTopRight, thickness);
                    }
                }
            }

            territoryBoundaryMesh.Clear();
            territoryBoundaryMesh.SetVertices(vertices);
            territoryBoundaryMesh.SetTriangles(triangles, 0);
            territoryBoundaryMesh.RecalculateBounds();
        }

        bool IsPlayerOwned(int x, int y)
        {
            return x >= 0 && x < width && y >= 0 && y < height && owners[x, y] == TileOwner.Player;
        }

        void DestroyTerritoryBoundaryOverlay()
        {
            if (territoryBoundaryOverlay == null)
            {
                var existing = transform.Find("Territory Boundary Overlay");
                if (existing != null) DestroyUnityObject(existing.gameObject);
            }

            if (territoryBoundaryOverlay != null) DestroyUnityObject(territoryBoundaryOverlay);
            if (territoryBoundaryMesh != null) DestroyUnityObject(territoryBoundaryMesh);
            if (territoryBoundaryMaterial != null) DestroyUnityObject(territoryBoundaryMaterial);
            territoryBoundaryOverlay = null;
            territoryBoundaryMesh = null;
            territoryBoundaryMaterial = null;
            territoryBoundaryDirty = false;
        }
